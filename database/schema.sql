...
-- F. FIRST PILOT CUSTOMER SEED (TOCCO Exports, Tiruppur)
INSERT INTO `companies` (`id`, `name`, `subdomain`, `email`, `phone`, `address`, `city`, `state`, `country`, `postal_code`, `gstin`, `status`, `subscription_plan_id`, `subscription_status`, `subscription_expires_at`) VALUES
(1, 'TOCCO Exports', 'tocco', 'info@toccoexports.com', '919876543210', '123 Garment Avenue, Avinashi Road', 'Tiruppur', 'Tamil Nadu', 'India', '641602', '33AAAAT1234A1Z1', 'active', 4, 'active', '2036-07-21 00:00:00');

-- G. SEED TENANT ROLES FOR TOCCO EXPORTS
INSERT INTO `roles` (`id`, `company_id`, `name`, `description`, `is_system`) VALUES
(2, 1, 'Company Admin', 'Administrator for TOCCO Exports ERP tenant', 1),
(3, 1, 'Production Manager', 'Responsible for knitting, sewing, compacting steps', 0),
(4, 1, 'Merchandiser', 'Responsible for style master, fabric specifications, client quotes', 0);

-- H. MAP TENANT ROLES TO PERMISSIONS
-- Company Admin gets all tenant-specific permissions (ids 8-16)
INSERT INTO `role_permissions` (`role_id`, `permission_id`)
SELECT 2, id FROM `permissions` WHERE `module` = 'tenant';

-- Production Manager gets dashboard & basic settings
INSERT INTO `role_permissions` (`role_id`, `permission_id`) VALUES
(3, 8); -- company.dashboard

-- Merchandiser gets dashboard & basic settings
INSERT INTO `role_permissions` (`role_id`, `permission_id`) VALUES
...
(1, 'rfid', 'trial'),
(1, 'ai_reports', 'beta'),
(1, 'customer_portal', 'disabled'),
(1, 'vendor_portal', 'disabled');

-- K. SEED PILOT CUSTOMER LICENSE
INSERT INTO `license_keys` (`company_id`, `license_key`, `status`, `expires_at`) VALUES
(1, 'LIC-TOCCO-EXP-2026-9999-LIFETIME', 'active', '2036-07-21 00:00:00');
